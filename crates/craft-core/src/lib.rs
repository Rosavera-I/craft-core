...
pub fn compose_harnesses(
    ...
    for name in harness_names {
        ...
        note_conflict(
            &mut memory_schemas,
            &manifest.memory.schema.to_string_lossy(),
            &manifest.harness.name,
            &mut warnings,
        );
        note_conflict(
            &mut tool_bindings,
            &manifest.tools.mcp.to_string_lossy(),
            &manifest.harness.name,
            &mut warnings,
        );
        entries.push((installed, manifest));
    }

    let contents = render_compose(&entries, &warnings);
...
fn render_compose(entries: &[(InstalledHarness, Manifest)], warnings: &[String]) -> String {
    let mut output = String::new();
    output.push_str("# Generated by craft compose\n\n");
    output.push_str("[compose]\n");
    output.push_str("strategy = \"last-write-wins\"\n");
    output.push_str("harnesses = [");
    for (index, (_, manifest)) in entries.iter().enumerate() {
        if index > 0 {
            output.push_str(", ");
        }
        output.push_str(&quoted(&manifest.harness.name));
    }
    output.push_str("]\n\n");
    ...
    for (installed, manifest) in entries {
        output.push_str("[[harness]]\n");
        output.push_str("name = ");
        output.push_str(&quoted(&manifest.harness.name));
        output.push('\n');
        output.push_str("version = ");
        output.push_str(&quoted(&manifest.harness.version));
        output.push('\n');
        output.push_str("source = ");
        output.push_str(&quoted(&installed.source));
        output.push('\n');
        output.push_str("path = ");
        output.push_str(&quoted(&installed.path.to_string_lossy()));
        output.push_str("\n\n");
    }
...
mod tests {
    ...
    fn compose_writes_merged_config_with_warnings() {
        ...
        assert!(contents.contains("strategy = \"last-write-wins\""));
    ...
    fn create_harness(root: &Path, name: &str) {
        ...
        fs::write(harness_root.join("prompts/system.md"), "").unwrap_or_else(|err| panic!("{err}"));
        fs::write(harness_root.join("memory/schema.toml"), "")
            .unwrap_or_else(|err| panic!("{err}"));
        fs::write(harness_root.join("tools/mcp.toml"), "").unwrap_or_else(|err| panic!("{err}"));
        fs::write(harness_root.join("validators/checks.tdd"), "")
            .unwrap_or_else(|err| panic!("{err}"));
        fs::write(
